...
pub fn process(opt: Options) !void {
    ...
    const fd = try std.Io.Dir.cwd().openFile(
        opt.io,
        opt.filepath,
        .{},
    );
    ...
    const root_out_dir = if (opt.root_out_dir) |rod|
        try std.Io.Dir.cwd().createDirPathOpen(opt.io, rod, .{})
    else
        null;
    ...
    while (it.next() catch |err| {
        ...
        if (root_out_dir) |rod| {
            if (std.mem.startsWith(u8, entry.name, "build/root/")) {
                ...
                switch (entry.kind) {
                    .directory => try rod.createDirPath(opt.io, entry_short_name),
                    .file => {
                        if (std.fs.path.dirname(entry_short_name)) |out_dir_name| {
                            try rod.createDirPath(opt.io, out_dir_name);
                        }
                        const out_fd = try rod.createFile(opt.io, entry_short_name, .{});
                        defer out_fd.close(opt.io);
                        var out_writer = out_fd.writer(opt.io, &.{});
                    ...
                    .sym_link => {
                        ...
                        try rod.symLink(opt.io, entry_short_name, entry.link_name, .{});
                    },
                }
                continue;
            }
        ...
        if (!gop.found_existing) {
            ...
            gop.value_ptr.*.filew = gop.value_ptr.*.tf.f.writer(opt.io, &gop.value_ptr.*.tarbuf);
...

    {
        ...
        while (vit.next()) |a| {
            ...
            const res = try std.process.run(opt.gpa, opt.io, .{
                .argv = &.{
